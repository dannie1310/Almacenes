...
public class TransferenciaDetalle {
    ...
    public static List<TransferenciaDetalle> getTransferencia(Context context){
        DBScaSqlite db_sca = new DBScaSqlite(context, "sca", null, 1);
        SQLiteDatabase db = db_sca.getWritableDatabase();
        Cursor c = db.rawQuery("SELECT * FROM transferenciadetalle ORDER BY fecha", null);
        ArrayList sa = new ArrayList<TransferenciaDetalle>();
        try {
            if (c != null){
                while (c.moveToNext()){
                    TransferenciaDetalle s = new TransferenciaDetalle(context);
                    s = s.findT(c.getInt(0));
                    sa.add(s);
                }
               /* Collections.sort(sa, new Comparator<TransferenciaDetalle>() {
                    @Override
                    public int compare(TransferenciaDetalle v1, TransferenciaDetalle v2) {
    ...
    public TransferenciaDetalle findT (Integer id) {
        db = db_sca.getWritableDatabase();
        Almacen a = new Almacen(context);
        Cursor c = db.rawQuery("SELECT * FROM transferenciadetalle td INNER JOIN transferencia t ON t.id = td.idtransferencia LEFT JOIN almacenes a ON td.idalmacenDestino = a.id_almacen LEFT JOIN contratistas c ON c.idempresa = td.idContratista INNER JOIN materiales m ON m.id_material = td.idmaterial WHERE td.id='"+id+"'", null);
        try {
            if (c != null && c.moveToFirst()) {

                this.cantidad =c.getDouble(4);
                this.idalmacenOrigen = c.getString(2);
                this.idalmacenDestino = c.getString(18);
                this.cargo = c.getInt(7);
                this.unidad = c.getString(8);
                this.fecha =c.getString(9);
                this.almacenOrigen = a.getIdAlmacen(c.getInt(2));
                this.referencia = c.getString(12);
                this.observacion = c.getString(13);
                this.idobra = c.getInt(15);
                this.folio = c.getString(16);
                this.almacenDestino = c.getString(18);
